...
def try_llama_main(ngl, llama_main_path, model_path, desired_context,
  ...
  cmd = f'{llama_main_path} -m {model_path} -c {desired_context} --split-mode {split_mode}{tensor_split} --main-gpu {main_gpu} --flash-attn auto --no-conversation --single-turn -t 4 -ngl {ngl} -n 1 --prompt "if this sentence is in stdout then our ngl was ok"'
  process = subprocess.Popen(cmd, stdin=subprocess.DEVNULL, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
...
if len(gpu_names) == 0:
  print('No NVIDIA GPUs detected via nvidia-smi. Falling back to single-GPU mode with --main-gpu 0.')
  gpu_names = ['GPU0']
  if len(vram_per_gpu) == 0:
    vram_per_gpu = [0]
  if len(pcie_lanes_per_gpu) == 0:
...
print(f'\nRun llama.cpp with these arguments:\n-m {model_path} -c {desired_context} --split-mode {split_mode}{tensor_split} --main-gpu {main_gpu_ind} --flash-attn auto -t 4 -ngl {max_ngl_possible}')
